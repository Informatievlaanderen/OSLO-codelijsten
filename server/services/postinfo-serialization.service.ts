...
const implementatie = ns('https://implementatie.data.vlaanderen.be/ns/adres#')
...
export const postinfoDataToQuads = (data: PostinfoData): RDF.Quad[] => {
  const quads: RDF.Quad[] = []
  const subject = df.namedNode(data.uri)

  // --- rdf:type ---
  quads.push(df.quad(subject, rdf('type'), adres('Postinfo')))
  ...
  // --- Postcode (the id) ---
  addLiteral(quads, subject, adres('postcode'), data.postcode)

  // --- Postnaam ---
  addLiteral(quads, subject, adres('postnaam'), data.postnaam, 'nl')

  // --- Status ---
  addNamedNode(quads, subject, implementatie('Postinfo.status'), data.status?.uri)

  // --- Nuts3 ---
  addLiteral(quads, subject, implementatie('nuts3'), data.nuts3)

  // --- Is toegekend aan (Gemeente) ---
  if (data.isToegekendAan?.uri) {
    const gemeenteNode = df.namedNode(data.isToegekendAan.uri)
    ...
    addLiteral(
      ...
      gemeenteNode,
      rdfs('seeAlso'),
      data.isToegekendAan.detail,
      xsd('anyURI'),
    )
    if (data.isToegekendAan.label) {
