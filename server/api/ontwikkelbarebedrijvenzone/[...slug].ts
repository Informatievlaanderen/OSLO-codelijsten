import { getOntwikkelbareBedrijvenzone, getSparqlEndpoint } from '~/server/services/bedrijventerrein.service'
import { SUPPORTED_EXTENSIONS, SUPPORTED_FORMATS } from '~/constants/constants'
import { ONTWIKKELBAREBEDRIJVENZONE_URI_BASE } from '~/constants/bedrijventerrein.constants'
...
export default defineEventHandler(
  async (event): Promise<OntwikkelbareBedrijvenzone | string | null> => {
    try {
      ...
      console.log(
        `[${new Date().toISOString()}] Fetching ontwikkelbarebedrijvenzone: ${slug}`,
      )
      ...
      if (requestedFormat) {
        const entityUri = `${ONTWIKKELBAREBEDRIJVENZONE_URI_BASE}${cleanSlug}`
      ...
      const data = await getOntwikkelbareBedrijvenzone(cleanSlug)

      if (!data) {
        throw createError({
          statusCode: 404,
          statusMessage: `Ontwikkelbarebedrijvenzone not found: ${cleanSlug}`,
        })
      }
      ...
      throw createError({
        statusCode: 500,
        statusMessage: 'Error fetching ontwikkelbarebedrijvenzone',
      })
    }
  },
)
