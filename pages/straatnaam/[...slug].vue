        ...
        <vl-column width="12">
          <div class="h1-sublink">
            <vl-title mod-no-space-bottom tag-name="h1">
              {{ data?.straatnaam ?? `Straatnaam: ${slug}` }}
            </vl-title>
        ...
        <vl-column width="12">
          <vl-data-table>
            <tbody>
              ...
              <tr v-if="data?.homoniemToevoeging?.length">
                <td>
                  <vl-link :href="data.fieldUris.homoniemToevoeging" external>
                    Homoniem toevoeging
                  </vl-link>
                </td>
                <td>
                  <div
                    v-for="homoniem in data.homoniemToevoeging"
                    :key="homoniem"
                  >
                    {{ homoniem }}
                  </div>
              ...
              <tr v-if="data?.isToegekendDoor">
                ...
                <td>
                  <vl-link
                    v-if="data.isToegekendDoor.uri"
                    :href="`/doc/gemeente/${extractId(data.isToegekendDoor.uri)}`"
                  >
                    {{ data.isToegekendDoor.label ?? data.isToegekendDoor.uri }}
                  </vl-link>
                </td>
              </tr>
              <tr v-if="data?.status">
...
const { data } = await useAsyncData<StraatnaamData | null>(
  `straatnaam-${slug.value}`,
  async () => {
    try {
      return await $fetch(`/doc/api/straatnaam/${slug.value}`)
    } catch (err) {
      console.error('Error loading straatnaam:', err)
      return null
    }
  },
)
...
if (!data?.value) {
  throw createError({
    statusCode: 404,
    statusMessage: 'Straatnaam niet gevonden',
  })
}

useSeoHead({
  title: data.value?.straatnaam ?? `Straatnaam: ${slug.value}`,
  description: `Straatnaam ${data.value?.straatnaam ?? slug.value}`,
})
</script>
